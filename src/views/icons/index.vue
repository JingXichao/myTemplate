...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async fetchData() {
        let iconList = this.allIcon.filter((item) => {
          if (
            this.queryForm.keyword &&
            item.indexOf(this.queryForm.keyword) < 0
          )
            return false
          return true
        })
        this.queryIcon = iconList.filter(
          (item, index) =>
            index < this.queryForm.pageSize * this.queryForm.pageNo &&
            index >= this.queryForm.pageSize * (this.queryForm.pageNo - 1) &&
            item.includes(this.queryForm.keyword)
        )
        this.total = iconList.length
      },
      handleCopyIcon(index, event) {
        const copyText = `<vab-icon :icon="['fas', '${this.queryIcon[index]}']" />`
